...
Executing (default): CREATE TABLE IF NOT EXISTS `user_detail` (`id` INTEGER PRIMARY KEY AUTOINCREMENT, `name` VARCHAR(255), `mobile` INTEGER, `state_id` INTEGER NOT NULL REFERENCES `state` (`id`) ON DELETE NO ACTION ON UPDATE CASCADE, `district_id` INTEGER NOT NULL REFERENCES `district` (`id`) ON DELETE NO ACTION ON UPDATE CASCADE, `area` VARCHAR(255), `streetAddress` VARCHAR(255), `created_at` DATETIME NOT NULL, `updated_at` DATETIME NOT NULL, `deleted_at` DATETIME);
...
`seller_id` INTEGER NOT NULL REFERENCES `user_detail` (`id`) ON DELETE NO ACTION ON UPDATE CASCADE, `status_id` INTEGER DEFAULT 1,
